...
function warnInvalid(key: string, expected: string, got: unknown): void {
  process.stderr.write(
    `OpenGate: ignoring config.json key "${key}": expected ${expected}, got ${typeof got}\n`,
  )
}
...
export function loadConfig(opts: LoadOptions = {}): LoadedConfig {
  ...
  try {
    ...
    try {
      ...
      process.stderr.write(
        `OpenGate: failed to parse ${path} (${(err as Error).message}); using defaults\n`,
      )
    }
  } catch (err: unknown) {
    if ((err as NodeJS.ErrnoException).code !== "ENOENT") {
      process.stderr.write(
        `OpenGate: failed to read ${path} (${(err as Error).message}); using defaults\n`,
      )
    }
  }
