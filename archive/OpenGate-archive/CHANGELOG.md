...
- Codex requests can now use `gpt-5.3-codex-spark` as a supported model. ([#14](https://github.com/raine/opengate/pull/14))
...
- Added `config.json` as an alternative to environment variables (read from
  `~/.config/opengate/config.json` on macOS, XDG-compliant on Linux)
...
- Codex now sends a default `User-Agent: OpenGate/<version>` header

## v0.0.7 (2026-04-25)

- Some security hardening inspired by [#5](https://github.com/raine/opengate/pull/5)
...
- Added `CCP_CODEX_MODEL` and `CCP_CODEX_EFFORT` environment variables to
  ...
  ([#2](https://github.com/raine/opengate/pull/2))
...
- Renamed to `opengate` to reflect provider-agnostic gateway support
