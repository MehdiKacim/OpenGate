#!/usr/bin/env bash
#
# opengate installation script
# Usage: curl -fsSL https://raw.githubusercontent.com/raine/opengate/main/scripts/install.sh | bash
#
# Environment variables:
#   OPENGATE_VERSION      - Pin a specific version (e.g., v0.1.0)
#   OPENGATE_INSTALL_DIR  - Override install directory (default: /usr/local/bin or ~/.local/bin)
#
# Examples:
#   OPENGATE_VERSION=v0.1.0 bash install.sh
#   OPENGATE_INSTALL_DIR=/opt/bin bash install.sh
#
...
install_from_release() {
	...
	local version="${OPENGATE_VERSION:-}"
	...
	if [ -z "$version" ]; then
		...
		if [ -z "$version" ]; then
			...
			echo "  OPENGATE_VERSION=v0.1.0 bash install.sh"
			echo ""
			exit 1
		fi
	fi
	...
	local install_dir="${OPENGATE_INSTALL_DIR:-}"
